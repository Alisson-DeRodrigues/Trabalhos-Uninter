...
-- quantitativo de livros cadastrados independente da editora
select count(idLivro) as 'Total de Livros' 
from Livro;

-- listagem dos nomes dos clientes cadastrados em ordem crescente
select nome as 'Clientes Cadastrados'
from Cliente order by nome asc;

-- listagem do nome de todas as editoras e os títulos de seus respectivos livros em ordem decrescente por nome de editora
select  nome as 'Editora', titulo
from Livro
inner join Editora
on Livro.idEditora = Editora.idEditora
order by nome desc;

-- listagem do nome das editoras e a média de preço de seus respectivos livros
select nome as 'Editora', avg(preco) as 'Média do Preço do Livro'
from Editora
inner join Livro
on Editora.idEditora = Livro.idEditora
group by nome;

-- listagem do nome de todos os clientes e a quantidade de livros comprados por eles
select nome as 'Cliente', count(idLivro) as 'Nº de Livros Comprados'
from Cliente
left join Pedido on Cliente.idCliente = Pedido.idCliente
left join ItemPedido on Pedido.idPedido = ItemPedido.idPedido
group by nome;
